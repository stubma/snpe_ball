...
public:
  ...
  DlSystem::ErrorCode add(const char* name, void* address, int32_t fd, uint64_t offset) noexcept{
    return static_cast<DlSystem::ErrorCode>(Snpe_UserMemoryMap_AddFdOffset(handle(), name, address, fd, offset));
  }
